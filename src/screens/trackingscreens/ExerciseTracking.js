...
function ExerciseTracking() {
  ...
  const displayDateText = () => {
    if (Platform.OS === 'ios') {
      if (
        format(new Date(), 'MM-DD') === format(new Date(state.date), 'MM-DD')
      ) {
        return 'Today';
      } else {
  ...
  const submitForm = React.useCallback(async () => {
    const exerciseRef = scopeRefByUserAndDate(
      'Surveys',
      'exercise',
      Platform.OS === 'android' ? date : state.date,
    );
    ...
    if (didFollowFID === undefined) {
      ...
      //replace Actions with navigate()
      Alert.alert(
        'Success!',
        `Your exercise for ${displayDateText()} has been recorded.`,
          ...
          <TouchableOpacity
            onPress={() =>
              setState(prevState => ({
                ...
                showAndroid: Platform.OS === 'android' ? true : false,
              }))
            }
            style={{
              height: 30,
            ...
            <Text style={{color: '#fff'}}>{displayDateText()}</Text>
          </TouchableOpacity>
